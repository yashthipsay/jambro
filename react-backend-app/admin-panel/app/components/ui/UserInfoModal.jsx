        ...
        {userInfo && (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-gray-200">Status: {userInfo.bookingInfo?.status}</p>
              <p className="text-gray-200">Session Date: {new Date(userInfo.bookingInfo?.date).toLocaleDateString()}</p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {userInfo.bookingInfo?.slots?.map((slot, index) => (
                  <div key={index} className="bg-black/20 p-2 rounded">
                    <p className="text-gray-200">{slot.startTime} - {slot.endTime}</p>
                  </div>
                ))}
              </div>
            </div>
