        ...
        if (data.success) {
          ...
          const events = filteredBookings.map((booking) => {
            ...
            const sortedSlots = [...booking.slots].sort((a, b) => {
              return (
                moment(a.startTime, 'HH:mm') - moment(b.startTime, 'HH:mm')
              );
            });
            ...
            >
              <CardContent className="glass-card p-4">
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold text-[#7DF9FF]">
                    Booking #{booking.id.slice(-4)}
                  </h3>
                  <span
                    className={`px-2 py-1 rounded text-sm ${
                      booking.extendedProps.status === 'NOT_STARTED'
                        ? 'bg-yellow-500'
                        : booking.extendedProps.status === 'ONGOING'
                          ? 'bg-blue-500'
