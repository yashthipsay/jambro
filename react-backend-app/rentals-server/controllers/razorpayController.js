  ...
  try {
    ...
    try {
      ...
      const stakeholderPayload = {
        ...
        relationship: stakeholderDetails.relationship || { director: true, executive: true },
        phone: {
          primary: String(stakeholderDetails.phone),
          secondary: stakeholderDetails.secondary_phone ? String(stakeholderDetails.secondary_phone) : ''
        },
      ...
      stakeholder = await razorpay.stakeholders.create(linkedAccount.id, stakeholderPayload);
    ...
    try {
      ...
      console.log('✅ Product config requested successfully:', productConfig.id);
      // Store success state
      if (shopId) {
          ...
          shopId,
          {
            $set: {
              'razorpay.route_config': {
                ...
                status: 'failed',
                error: productError.message || 'Failed to request product configuration'
              }
            }
          },
          { new: true }
...
export const updateBankDetails = async (req, res) => {
  try {
    ...
    const { ifsc, account_number, beneficiary_name, product_id, shopId } = req.body;

    if (!ifsc || !account_number || !beneficiary_name || !product_id) {
      return res.status(400).json({
        success: false,
        error: 'IFSC code, account number, beneficiary name, and product_id are required'
      });
    }
    ...
    if (req.body.shopId) {
      await RentalShop.findByIdAndUpdate(req.body.shopId, {
        $set: {
          ...
          'bank.verification_error': err.response?.data?.error?.description || err.message,
          'bank.last_verification_attempt': new Date(),
          'razorpay.route_config.bank_verification_error': err.response?.data?.error?.description || err.message
        }
      }).catch(console.error);
    }
    ...
    if (err.response?.status === 400) {
      return res.status(400).json({
        success: false,
        error: err.response.data.error?.description || 'Bank details validation failed'
      });
    }
...
export const updateLinkedAccount = async (req, res) => {
  try {
    ...
    const updatePayload = {
      ...
      ...updateData.legal_business_name && { legal_business_name: updateData.legal_business_name },
      ...updateData.customer_facing_business_name && { customer_facing_business_name: updateData.customer_facing_business_name },
    ...
    // Update in Razorpay
    const updatedAccount = await razorpay.accounts.edit(account_id, updatePayload);
      ...
      await RentalShop.findByIdAndUpdate(updateData.shopId, {
        $set: {
          ...
          'razorpay.account_details': {
            ...
            customer_facing_business_name: updatedAccount.customer_facing_business_name,
...
export const checkAccountStatus = async (req, res) => {
  try {
    ...
    const shop = await RentalShop.findOne({ 'razorpay.linked_account_id': account_id });
    ...
    if (shop?.razorpay?.route_config?.product_id) {
      try {
        ...
        productStatus = shop.razorpay?.route_config?.status || 'error_checking_products';
      }
    } else {
    ...
    const isReadyForTransfers = account.status === 'activated' && productStatus === 'activated';
...
export const createOrder = async (req, res) => {
  try {
    ...
    const rentalOwnerSharePaise = totalAmountPaise - depositPaise - platformFeePaise;
...
try{
    ...
    const valid = svcVerify(payload, razorpay_signature, process.env.RAZORPAY_KEY_SECRET);
    if (!valid) return res.status(400).json({ success: false, error: 'Invalid signature' });

    const payment = await razorpay.payments.fetch(razorpay_payment_id);
    if (payment.status !== 'captured') {
      return res.status(400).json({ success: false, error: 'Payment not captured', payment_status: payment.status });
    }

    const transfers = await razorpay.transfers.all({ payment_id: razorpay_payment_id });
    res.json({ success: true, payment_id: razorpay_payment_id, status: payment.status, transfers: transfers.items });
  } catch (err) {
...
export const getTransfers = async(req, res) => {
  try {
    const transfers = await razorpay.transfers.all({ payment_id: req.params.payment_id });
